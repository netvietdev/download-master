...
{
    public class DhhnDownloadService : PageFilesDownloadService
    {
        ...
        {
            return RegexHelper.GetLinks(pageContent).Where(link => Regex.IsMatch(link, filePattern)).Select(x => "http://www.hanu.vn/" + x).Distinct();
        }
    }
}