...
{
    public class DhyhnDownloadService : PageFilesDownloadService
    {
        ...
        {
            return RegexHelper.GetLinks(pageContent).Where(link => Regex.IsMatch(link, filePattern)).Select(x => "http://www.hmu.edu.vn" + x).Distinct();
        }
    }
}